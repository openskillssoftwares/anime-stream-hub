        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
          <div className="flex flex-wrap gap-1">
            {item.genres?.slice(0, 2).map((g) => (
              <span key={g.name} className="text-[10px] bg-primary/90 text-primary-foreground rounded px-1.5 py-0.5 leading-none">
                {g.name}
              </span>
            ))}
          </div>
...
const Trending = () => {
  useEffect(() => {
    document.title = "Trending — Hey Anime";
    const meta = document.querySelector('meta[name="description"]');
    if (meta) meta.setAttribute("content", "What everyone is watching right now.");
  }, []);

  const { data, isLoading } = useQuery<AnimeItem[]>({
    queryKey: ["trending"],
    queryFn: jikan.trending,
    staleTime: 5 * 60_000,
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container pt-28 pb-24">
        ...
        <header className="relative mb-14 overflow-hidden">
          ...
          <div className="relative z-10 pt-6">
            ...
            <p className="mt-4 text-muted-foreground max-w-xl leading-relaxed">
              The titles everyone can't stop talking about — updated daily.
            </p>
          </div>
        ...
        <div className="my-16 max-w-3xl mx-auto">
          <AdSlot slot="trending-mid" className="mt-6" />
        </div>
      </main>
    </div>
  );
