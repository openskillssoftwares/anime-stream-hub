...
export const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-40 glass border-b border-border/40">
      <div className="container flex items-center gap-2 sm:gap-4 h-18">
        <Link to="/" className="flex items-center gap-2 sm:gap-3 group shrink-0">
          <Sparkles className="w-8 h-8 sm:w-10 sm:h-10 text-primary transition-transform group-hover:rotate-12" />
          <span className="font-display text-lg sm:text-xl font-semibold tracking-tight hidden sm:block">
        ...
        <nav className="hidden xl:flex items-center gap-5 text-sm text-muted-foreground">
          ...
          <Link to="/trending" className="hover:text-foreground transition-colors inline-flex items-center gap-2">
            <Flame className="w-5 h-5" /> Trending
          </Link>
          <Link to="/new" className="hover:text-foreground transition-colors inline-flex items-center gap-2">
            <Sparkles className="w-5 h-5" /> New
          </Link>
          <Link to="/season" className="hover:text-foreground transition-colors inline-flex items-center gap-2">
            <CalendarDays className="w-5 h-5" /> This Season
          </Link>
          <Link to="/top10" className="hover:text-foreground transition-colors inline-flex items-center gap-2">
            <Trophy className="w-5 h-5" /> Top 10
          </Link>
        </nav>

        {/* Responsive Search */}
        <form onSubmit={onSearch} className="flex-1 w-full max-w-md mx-2 sm:mx-4 xl:ml-auto relative">
          ...
          <Input
            ...
            className="pl-9 bg-secondary/50 border-border/60 focus-visible:ring-primary w-full"
          />
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <Button variant="outline" size="sm" onClick={goRandom} className="hidden md:inline-flex items-center gap-2 px-4 shrink-0" title="Random anime">
          ...
          {user ? (
            ...
            <Button asChild variant="default" size="sm" className="bg-gradient-ember text-primary-foreground hover:opacity-90 shadow-glow hidden xs:inline-flex">
      ...
      {mobileMenuOpen && (
        <div className="xl:hidden absolute top-full left-0 right-0 bg-background/95 backdrop-blur-xl border-b border-border/40 p-4 flex flex-col gap-2 shadow-2xl z-50">
          <nav className="flex flex-col">
            <Link to="/browse" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <Compass className="w-5 h-5 text-muted-foreground" /> Browse
            </Link>
            <Link to="/community" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <Dices className="w-5 h-5 text-muted-foreground" /> Community
            </Link>
            <Link to="/rooms" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <Video className="w-5 h-5 text-muted-foreground" /> Rooms
            </Link>
            <Link to="/trending" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <Flame className="w-5 h-5 text-muted-foreground" /> Trending
            </Link>
            <Link to="/new" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <Sparkles className="w-5 h-5 text-muted-foreground" /> New
            </Link>
            <Link to="/season" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <CalendarDays className="w-5 h-5 text-muted-foreground" /> This Season
            </Link>
            <Link to="/top10" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors">
              <Trophy className="w-5 h-5 text-muted-foreground" /> Top 10
            </Link>
            <button onClick={goRandom} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm text-left transition-colors md:hidden">
              <Dices className="w-5 h-5 text-muted-foreground" /> Random Anime
            </button>
            {!user && (
              <Link to="/auth" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 hover:bg-secondary/50 rounded-lg text-sm transition-colors xs:hidden text-primary font-medium">
